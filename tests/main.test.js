...
describe(projectName, () => {
  beforeAll(async () => {
    browser = await puppeteer.launch({ headless: false , slowMo:70 }) //change to false if you want to view the page
  ...
  skip('User should be able to save and load their tasks from the api and save it to the local storage', async () => {
    const initialTasksCount = await countTasks()
    expect(initialTasksCount).toBe(3)
  ...
  skip('User should be save tasks from the api', async () => {
    await addTasksAndTest()
    const initialTasksCount = await countTasks()
    expect(initialTasksCount).toBe(3)
